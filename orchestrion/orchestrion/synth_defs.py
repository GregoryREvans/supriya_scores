import supriya
from orchestrion.session_setup import channel_count
...
with supriya.SynthDefBuilder(
    ...
    sig = supriya.ugens.BufRd.ar(channel_count=2, buffer_id=builder['buffer_id'], phase=ptr)
    sig = sig * env
    sig = supriya.ugens.Pan2.ar(source=sig, position=supriya.ugens.LFNoise2.kr(frequency=0.05), level=builder['amp'])
    sig = supriya.ugens.XFade2.ar(
