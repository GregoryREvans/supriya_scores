...
warp_buffer_player_pattern_1_prime = patterns.Pbind(
    add_action=AddAction.ADD_TO_HEAD,
    buffer_id=patterns.Prand(buffers, repetitions=None),
    delta=patterns.Pwhite(0.0, 7.0),
    duration=0,
    gain=patterns.Pwhite(-24, -12),
    overlaps=patterns.Prand([23, 41], repetitions=None),
    pan=patterns.Pwhite(-1.0, 2.0),
    rate=patterns.Pwhite(0.0625, 9),
    synthdef=warp_buffer_player_synthdef,
    transpose=patterns.Pwhite(-9.0, -1.0),
    window_size=0.25,
    )
...
warp_buffer_player_pattern_2 = patterns.Pbind(
    ...
    synthdef=warp_buffer_player_synthdef,
    transpose=patterns.Pwhite(-12.0, 6.0),
    window_size=0.25,
    )
...
warp_buffer_player_pattern_3 = patterns.Pbind(
    add_action=AddAction.ADD_TO_HEAD,
    buffer_id=patterns.Prand(buffers, repetitions=None),
    delta=patterns.Pwhite(0.0, 10.0),
    duration=0,
    gain=patterns.Pwhite(-24, -12),
    overlaps=patterns.Prand([20, 32], repetitions=None),
    pan=patterns.Pwhite(-1.0, 2.0),
    rate=patterns.Pwhite(0.5, 2.25),
    synthdef=warp_buffer_player_synthdef,
    transpose=patterns.Pwhite(-10.0, 4.0),
    window_size=0.25,
    )
...
with SynthDefBuilder(
    ...
    source = ugens.LPF.ar(source=source, frequency=10000)
    ugens.ReplaceOut.ar(
        bus=builder['out'],
        source=source,
        )

lowpass = builder.build()
...
pattern_1_prime = patterns.Pbus(
    patterns.Ppar([
        warp_buffer_player_pattern_1_prime,
        allpass_pattern,
        freqshift_pattern,
        freeverb_pattern,
        chorus_pattern,
...
with session.at(0):
    for i in range(3):
        session.inscribe(pattern_1, duration=60 * 3.5, seed=i)
    session.add_synth(
        add_action=AddAction.ADD_TO_TAIL,
...
with session.at(180):
    for i in range(3):
        session.inscribe(pattern_1_prime, duration=60 * 3, seed=i)

with session.at(340):
    for i in range(3):
        session.inscribe(pattern_2, duration=60 * 7, seed=i)
    # session.add_synth(
    #     add_action=AddAction.ADD_TO_TAIL,
    #     synthdef=multiband_compressor,
    #     )
...
with session.at(760):
    for i in range(3):
        session.inscribe(pattern_3, duration=60 * 3, seed=i)
    # session.add_synth(
    #     add_action=AddAction.ADD_TO_TAIL,
    #     synthdef=multiband_compressor,
    #     )
